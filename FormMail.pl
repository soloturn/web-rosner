...
##############################################################################
#     Net::SMTP MODS: Darrin M. Gorski                                       #
#                     http://www.gorski.net/darrin/scripts/formmail          #
##############################################################################
...
sub return_html {
    ...
    # Otherwise, begin printing the response page.                           #
        ...
        <center><font size=-1><a href="http://www.scriptarchive.com/formmail.html">FormMail</a> V1.93+SMTP &copy; 1995 - 2009  Matt Wright<br>
...
sub send_mail {
    ...
    local($print_config,$key,$sort_order,$sorted_field,$env_report,@message,$mailer);

    push @message,  "To: $Config{'recipient'}\n";
    push @message,  "From: $Config{'email'} ($Config{'realname'})\n";
    push @message,  "X-Mailer: FormMail-1.93+SMTP\n";


    # Check for Message Subject
    if ($Config{'subject'}) { push @message,  "Subject: $Config{'subject'}\n\n" }
    else                    { push @message,  "Subject: WWW Form Submission\n\n" }

    push @message,  "Below is the result of your feedback form.  It was submitted by\n";
    push @message,  "$Config{'realname'} ($Config{'email'}) on $date\n";
    push @message,  "-" x 75 . "\n\n";

    if (@Print_Config) {
        foreach $print_config (@Print_Config) {
            if ($Config{$print_config}) {
                push @message,  "$print_config: $Config{$print_config}\n\n";
            }
        }
    }
    ...
    if ($Config{'sort'} =~ /^order:.*,.*/) {
        ...
        foreach $sorted_field (@sorted_fields) {
            if ($Config{'print_blank_fields'} || $Form{$sorted_field} ne '') {
                push @message,  "$sorted_field: $Form{$sorted_field}\n\n";
            }
        }
    }
    ...
    foreach $env_report (@Env_Report) {
        if ($ENV{$env_report}) {
            push @message,  "$env_report: $ENV{$env_report}\n";
        }
    }
    ...
    $mailer =
      Net::SMTP->new($mailhost, Hello => 'FormMail-1.93-SMTP', Timeout => 30);
    &error('smtp_failed') unless $mailer;

    $mailer->mail($Config{'email'}) || &error('smtp_failed');
    $mailer->to(split(/,/, $Config{'recipient'})) || &error('smtp_failed');
    $mailer->data(@message) || &error('smtp_failed');
    $mailer->quit;
  ...
  <center>
   ...
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><td>The form attempting to use
     ...
     <center><font size=-1>
      <a href="http://www.scriptarchive.com/formmail.html">FormMail</a> V1.93+SMTP &copy; 1995 - 2009  Matt Wright<br>
...
<html>
 <head>
  <title>FormMail v1.93+SMTP</title>
 </head>
 <body bgcolor=#FFFFFF text=#000000>
  <center>
   <table border=0 width=600 bgcolor=#9C9C9C>
   ...
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><th><tt><font size=+1>Copyright 1995 - 2009 Matt Wright<br>
        Version 1.93+SMTP - Released June 25, 2009<br>
...
(END ERROR HTML)
    ...
    elsif ($error eq 'smtp_failed') {
            print <<"(END ERROR HTML)";
Content-type: text/html

<html>
 <head>
  <title>Error: Problem Sending Message</title>
 </head>
 <body bgcolor=#FFFFFF text=#000000>
  <center>
   <table border=0 width=600 bgcolor=#9C9C9C>
    <tr><th><font size=+2>Error: Problem Sending Message</font></th></tr>
   </table>
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><td>There was a problem sending the email message to the server.
            Please try again later.
     <center><font size=-1>
      <a href="http://www.scriptarchive.com/formmail.html">FormMail</a> V1.93+SMTP &copy; 1995 - 2002  Matt Wright<br>
      A Free Product of <a href="http://www.scriptarchive.com/">Matt's Script Archive, Inc.</a>
     </font></center>
    </td></tr>
   </table>
